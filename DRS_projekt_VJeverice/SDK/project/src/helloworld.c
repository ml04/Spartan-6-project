...
int main()
{
    ...
    while(1){
    	do{
			...
			xil_printf("Igrac1_cilj: %d Lokacija1: %d\r\n", igrac1_cilj, location_x1);
			...
			for (i =0; i<4; i++){
				if(polje[i]==1){
					VGA_CONTROLLER_mWriteSlaveReg2(XPAR_VGA_CONTROLLER_0_BASEADDR,0,i+1);
				}
				if(polje[i]==2){
					VGA_CONTROLLER_mWriteSlaveReg4(XPAR_VGA_CONTROLLER_0_BASEADDR,0,i+1);
				}
				if (polje[i]==3){
					VGA_CONTROLLER_mWriteSlaveReg6(XPAR_VGA_CONTROLLER_0_BASEADDR,0,i+1);
				}
				if (polje[i]==4){
					VGA_CONTROLLER_mWriteSlaveReg8(XPAR_VGA_CONTROLLER_0_BASEADDR,0,i+1);
				}
			}
			...
			while (igrac1_cilj!=1 || igrac2_cilj!=2 || igrac3_cilj!=3 || igrac4_cilj!=4);
    }
